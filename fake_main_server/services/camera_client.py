...
async def get_photo(work_id: str) -> bytes:
    """Получает фото из сервиса камеры и возвращает его как байты."""
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            logger.error(f"Failed to take photo for work_id={work_id}, error: {e.text}")
